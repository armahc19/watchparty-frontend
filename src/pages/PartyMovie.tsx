...
interface PlaylistItem {
  ...
  type: string;
  duration: number;
  kind: 'video' | 'audio'; // Add this field
}

import { toast } from '@/hooks/use-toast';

const PartyMovie: React.FC = () => {
  ...
  const currentFileName = currentFile?.name || 'Loading media...';
  // WebSocket for reactions & typing
  //const { sendSyncMessage, lastMessage } = useWebSocket(roomId!);
  // This is CORRECT:
// Get WebSocket from the hook
const { sendSyncMessage, lastMessage, isConnected } = useWebSocket(roomId || '');
  ...
  const {
    ...
    onFileChanged: (fileData) => {
      console.log('AUDIENCE: onFileChanged received →', fileData.file_id);
      
      const targetFile = playlist.find(item => item.id === fileData.file_id);
      if (!targetFile) {
        console.log('File not in playlist:', fileData.file_id);
        return;
      }
      
      // Set new file with type detection
      setCurrentFile(null); // Clear first to force remount
  ...
  // Initial playlist with both video and audio
...
useEffect(() => {
  ...
  const loadPartyData = async () => {
    ...
    try {
      console.log('Loading party data for roomId →', roomId);
      ...
      if (!response.ok) {
        ...
        throw new Error(text || "Party not found or access denied");
      }
      
      const partyResult = await response.json();
      console.log("Party loaded successfully:", partyResult);
      
      if (partyResult.party.activity_type !== 'free') {
        toast({ title: "Wrong Room Type", description: "This is not a movie party room", variant: "destructive" });
        navigate('/join');
        return;
      }
      ...
      console.error("loadPartyData error:", error);
      toast({
  ...
  const handleUploadAndQueue = async (file: File) => {
    try {
      ...
      if (!userId || !roomId) {
        throw new Error("User not authenticated or no room ID");
      }
      ...
      toast({
        title: "Success!",
        description: `${file.name} uploaded and queued`,
      });
    } catch (error: any) {
      ...
      <header className="bg-gray-800 border-b border-gray-700 px-6 py-4">
        <div className="flex items-center justify-between">
          ...
          <div className="flex items-center space-x-4 ">
            ...
            {isHost ? (
              <div className="bg-green-900 text-green-400 px-4 py-2 rounded-lg font-semibold">
                🟢 HOST
              </div>
            ) : (
              <div className="bg-blue-900 text-blue-400 px-4 py-2 rounded-lg font-semibold mobile-controls">
                👀 FOLLOWING HOST
              </div>
            )}
          </div>
      ...
      {!isHost && !currentFile && (
                ...
                <div className="flex items-center justify-between">
                  ...
                  <div className="flex items-center space-x-4">
                    <div className={`px-4 py-2 rounded-full font-bold ${
                      syncStatus === 'synced' ? 'bg-green-900/80 text-green-400' : 
                      syncStatus === 'syncing' ? 'bg-yellow-900/80 text-yellow-400' : 
        ...
        <div className="h-full flex flex-col pt-16 md:pt-0 overflow-y-auto">
          ...
          <div className="flex-1 flex flex-col border-b border-gray-700">
            ...
            <input
                  ...
                  onChange={(e) => {
                    ...
                    sendSyncMessage({ type: 'typing_start', user: currentUser?.name || 'Guest' });

                    clearTimeout(typingTimeout);
                    typingTimeout = setTimeout(() => {
                      sendSyncMessage({ type: 'typing_stop', user: currentUser?.name || 'Guest' });
                    }, 1500);
                  }}
                  onKeyDown={(e) => {
          ...
          <div className="flex-1 flex flex-col min-h-0">
            ...
            <div className="h-12 flex-shrink-0 border-b border-gray-700 p-2">
              ...
              <div className="flex justify-between items-center">
                <span className="text-sm">
                  {currentFile?.kind === 'video' ? '🎬' : '🎵'} {currentFileName}
                </span>
            ...
            {/* Host Controls */}
            {isHost && (
              <div className="h-16 flex-shrink-0 border-b border-gray-700 p-2">
                <div className="text-xs text-gray-400 mb-1">UPLOAD MEDIA</div>
                <input
                  type="file"
                  accept="video/*,audio/*,.mp4,.mkv,.avi,.mp3,.wav,.ogg,.m4a"
                  onChange={(e) => {
                    const file = e.target.files?.[0];
                    if (file) handleUploadAndQueue(file);
                  }}
                  className="w-full text-sm text-gray-400 file:mr-4 file:py-2 file:px-4 file:rounded file:border-0 file:text-sm file:font-semibold file:bg-blue-50 file:text-blue-700 hover:file:bg-blue-100"
                />
              </div>
            )}
